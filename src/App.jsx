...
async function saveKey(key, value) {
  try { await window.storage.set(key, JSON.stringify(value)); } catch (e) { /* تجاهل */ }
}
    ...
    <aside className="h-screen sticky top-0 flex flex-col shrink-0 transition-all duration-300"
      ...
      <div className="flex items-center gap-2 px-4 py-5" style={{ borderBottom: `1px solid ${C.border}` }}>
        <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
          style={{ background: `linear-gradient(135deg, ${C.gold}, #8a6a30)` }}>
          <Gauge size={18} color="#0A0E17" />
        </div>
        {!collapsed && (
        ...
        {CATEGORIES.map((c) => (
          <button key={c.id} onClick={() => setCategory(c.id)}
            ...
            style={{
              ...
              color: category === c.id ? "#0A0E17" : C.dim,
...
function AssetDetail({ model, signal, price, changePct, inWatchlist, toggleWatch, goToPlan, live }) {
  ...
  const data = model.series.slice(-range).map((v, i) => ({ i, price: v }));
  const bcColor = signal.bias === "BUY" ? C.green : signal.bias === "SELL" ? C.red : C.gold;
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={goToPlan} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold"
            style={{ background: C.gold, color: "#0A0E17" }}>
      ...
      <Panel className="p-4 mb-5">
        <div className="flex items-center justify-between mb-2">
          <div className="flex gap-2">
            {[30, 90, 180].map((r) => (
              <button key={r} onClick={() => setRange(r)} className="px-3 py-1 rounded-lg text-xs font-semibold"
                style={{ background: range === r ? `${C.gold}22` : "transparent", color: range === r ? C.goldBright : C.dim, border: `1px solid ${range === r ? C.gold : C.border}` }}>
                {r} يوم
              </button>
            ))}
          </div>
          <span className="text-[11px]" style={{ color: C.dim }}>مصدر البيانات: <b style={{ color: C.gold }}>SIMULATED</b></span>
        </div>
        <ResponsiveContainer width="100%" height={340}>
          <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="priceFill" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={bcColor} stopOpacity={0.35} />
                <stop offset="100%" stopColor={bcColor} stopOpacity={0} />
              </linearGradient>
            </defs>
            <CartesianGrid stroke={C.border} strokeDasharray="3 3" vertical={false} />
            <XAxis dataKey="i" hide />
            <YAxis domain={["auto", "auto"]} tick={{ fill: C.dim, fontSize: 10 }} width={60}
              tickFormatter={(v) => fmtNum(v, model.decimals)} />
            <Tooltip contentStyle={{ background: C.bgPanel2, border: `1px solid ${C.border}`, borderRadius: 10, fontFamily: FONT_MONO, fontSize: 12 }}
              formatter={(v) => [fmtNum(v, model.decimals), "السعر"]} labelFormatter={() => ""} />
            <Area type="monotone" dataKey="price" stroke={bcColor} strokeWidth={2} fill="url(#priceFill)" isAnimationActive={false} />
            <ReferenceArea y1={Math.min(signal.entryLow, signal.entryHigh)} y2={Math.max(signal.entryLow, signal.entryHigh)} fill={C.cyan} fillOpacity={0.12} strokeOpacity={0} />
            {signal.sl && <ReferenceLine y={signal.sl} stroke={C.red} strokeDasharray="4 3" label={{ value: "SL", fill: C.red, fontSize: 10, position: "insideLeft" }} />}
            <ReferenceLine y={signal.tp1} stroke={C.green} strokeDasharray="4 3" label={{ value: "TP1", fill: C.green, fontSize: 10, position: "insideLeft" }} />
            {signal.tp2 && <ReferenceLine y={signal.tp2} stroke={C.green} strokeDasharray="2 2" label={{ value: "TP2", fill: C.green, fontSize: 10, position: "insideLeft" }} />}
            <ReferenceLine y={signal.support} stroke={C.dim} strokeDasharray="2 6" label={{ value: "دعم", fill: C.dim, fontSize: 10, position: "insideLeft" }} />
            <ReferenceLine y={signal.resistance} stroke={C.dim} strokeDasharray="2 6" label={{ value: "مقاومة", fill: C.dim, fontSize: 10, position: "insideLeft" }} />
          </AreaChart>
        </ResponsiveContainer>
        <p className="text-[11px] mt-2" style={{ color: C.dim }}>
          المنطقة السماوية: منطقة الدخول المقترحة من الذكاء الاصطناعي. الخطوط الذهبية المتقطعة: مستويات الدعم والمقاومة التقديرية.
        </p>
      </Panel>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        ...
        <Panel className="p-4">
          ...
          <button onClick={goToPlan} className="w-full mt-4 py-2.5 rounded-xl text-sm font-bold" style={{ background: C.gold, color: "#0A0E17" }}>
      ...
      <Panel className="p-4 mb-5">
        ...
        {!aiResult && !aiLoading && (
          <button onClick={runFullAI} className="w-full py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2"
            style={{ background: `linear-gradient(135deg, ${C.cyan}, ${C.gold})`, color: "#0A0E17" }}>
        ...
        <div className="flex gap-2">
          {["ALL", "BUY", "SELL", "WAIT"].map((f) => (
            <button key={f} onClick={() => setFilterBias(f)} className="px-3 py-1.5 rounded-full text-xs font-semibold"
              style={{ background: filterBias === f ? C.gold : "transparent", color: filterBias === f ? "#0A0E17" : C.dim, border: `1px solid ${filterBias === f ? C.gold : C.border}` }}>
        ...
        <Panel className="p-4 lg:col-span-1">
          ...
          <form onSubmit={submit}>
            ...
            <textarea placeholder="الأخطاء والدروس المستفادة" value={form.lessons} onChange={(e) => setForm({ ...form, lessons: e.target.value })}
              className="w-full mt-2 text-xs rounded-lg px-3 py-2" rows={2} style={{ background: C.bgDeep, color: C.softWhite, border: `1px solid ${C.border}` }} />
            <button type="submit" className="w-full mt-3 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2" style={{ background: C.gold, color: "#0A0E17" }}>
    ...
    <div>
      ...
      <Panel className="p-4 mb-5">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <button onClick={run} className="px-4 py-2 rounded-xl text-sm font-bold" style={{ background: C.gold, color: "#0A0E17" }}>تشغيل الاختبار</button>
        </div>
      </Panel>
      {result && (
        <>
    ...
    <div className="flex flex-col h-[calc(100vh-140px)]">
      ...
      <Panel className="flex-1 flex flex-col p-4 overflow-hidden">
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (input.trim() && !loading) send(input.trim()); }} className="flex gap-2">
          ...
          <button type="submit" disabled={loading} className="px-4 py-2.5 rounded-xl" style={{ background: C.gold, color: "#0A0E17" }}>
            <Send size={16} />
          </button>
        </form>
      </Panel>
